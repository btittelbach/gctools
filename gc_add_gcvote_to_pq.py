...
if __name__ == '__main__':
  ...
  try:
    opts, files = getopt.gnu_getopt(sys.argv[1:], "hu:p:", ["user=","pass=","help"])
  ...
  for gpxfile in files:
    ...
    votes_dict = getGCVotes(gcvote_username_, gcvote_password_, gcids)
    for wpt_elem in tree.iterfind("{http://www.topografix.com/GPX/1/0}wpt",namespaces=tree.nsmap):
      ...
      if sdesc_elem is not None and gccode in votes_dict:
        ...
        if sdesc_elem.get("html") == "False":
          sdesc_elem.text = vote_info+sdesc_elem.text
        else:
          votepelem = etree.Element("p",nsmap=tree.nsmap)
          votepelem.text=vote_info
          votepelem.append(etree.Element("br",nsmap=tree.nsmap))
          sdesc_elem.insert(0,votepelem)
